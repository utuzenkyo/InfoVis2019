function Isosurfaces( volume, isovalue, reflectionFlag, interpolationFlag, cmap)
{
	...
	if(reflectionFlag == 0 ){
		var material = new THREE.ShaderMaterial({
			...
			vertexShader: document.getElementById('lamb.vert').text,
			fragmentShader: document.getElementById('lamb.frag').text,
			uniforms:{
				light_position:{ type: 'v3', value: light.position},
				isovalue_color:{ type:'v3', value: new THREE.Color().setHex(cmap[isovalue][1])}
				}
		});
	}else if(reflectionFlag == 1){
        var material = new THREE.ShaderMaterial({
			vertexColors: THREE.VertexColors,
    ...
    function interpolated_vertex( v0, v1, s )
    {
		if(interpolationFlag == true){
			var s0 = volume.values[v0.x + v0.y * volume.numberOfNodesPerLine() + v0.z * volume.numberOfNodesPerSlice()][0];
			var s1 = volume.values[v1.x + v1.y * volume.numberOfNodesPerLine() + v1.z * volume.numberOfNodesPerSlice()][0];
			var t = (s - s0) / (s1 - s0);
